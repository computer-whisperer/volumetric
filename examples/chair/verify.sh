...
$V project-export -p chair_base.vproj --asset chair_model -o $O >/dev/null
cp "$E" $O/verify.vproj
$V project-add-model -p $O/verify.vproj -i $O/chair_model.wasm --asset-id chair_model >/dev/null

# Through three photographs: a low elevation, the top-down, an oblique.
for view in DSC00730 DSC00742 DSC00750; do
    $V render -i $O/verify.vproj --asset chair_model --through $view --overlay edge \
...
COMMON="--asset tsdf_cloud --asset chair_model --up 0,0,1 --projection ortho --ortho-scale 0.8 --width 1600 --height 1600 --grid 0 --no-ssao --resolution 256 -q --wireframe"

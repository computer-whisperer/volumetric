...
// Vendored addition (not in upstream 1.1.0): wasm32 with threads. Keyed on
// the wasi OS rather than `target_feature = "atomics"` — the feature cfg is
// unstable and never set on stable rustc, even for wasm32-wasip1-threads
// where the feature itself is on by default.

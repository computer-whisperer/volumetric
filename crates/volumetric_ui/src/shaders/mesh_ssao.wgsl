struct SsaoUniforms {
    ...
    bias: f32,
    strength: f32,
    // Uniform buffer structs are padded to 16-byte multiples.
    _pad0: vec2<f32>,
};
...
fn depth01_at_uv(uv: vec2<f32>) -> f32 {
    // `g_depth` is stored as a single-channel float texture (R32Float), which is not filterable.
    // Read it with `textureLoad` (integer pixel coords) instead of `textureSample`.
...
fn ndc_from_uv_depth(uv: vec2<f32>, depth01: f32) -> vec4<f32> {
    ...
    // Therefore, UV->NDC needs a Y flip.
...
fn fs_ssao(in: VsOut) -> @location(0) vec4<f32> {
    ...
    for (var s: i32 = 0; s < sample_count; s = s + 1) {
        ...
        // If the sampled depth is closer than the expected depth of our sample point, it occludes.
        // Compare in NDC depth space (0..1). Add bias to reduce self-shadowing.

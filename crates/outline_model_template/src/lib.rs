//! The filled-outline model template: a complete, *stateless* 2D Model ABI
//! implementation over an outline payload that `text_model_operator` (and
//! future contour-baking operators) patch into a copy of this module as
//! data segments.
//!
//! All contour work (font parsing, layout, Bézier flattening,
//! serialization) happens in the operator via `outline_model_core`; this
//! module only reads. Nothing here memoizes or mutates between `sample`
//! calls.
//!
//! # Patch contract
//!
//! - `outline_payload_slot() -> i32` returns the address of a 4-byte slot.
...
//! `crates/operators/text_model_operator/template/`). After changing this
//! crate or `outline_model_core`'s read side:
//!
//! ```text
...
#[unsafe(no_mangle)]
pub extern "C" fn outline_payload_slot() -> i32 {
...
pub extern "C" fn sample(pos_ptr: i32) -> f32 {
    ...
    match payload() {
        Some(view) if view.contains(p[0], p[1]) => 1.0,
        _ => 0.0,
    }
}
